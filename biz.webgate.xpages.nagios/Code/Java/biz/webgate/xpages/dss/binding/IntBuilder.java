...
public class IntBuilder implements IBinder<Integer> {

	private static IntBuilder m_Binder;
	...
	public static IBinder<Integer> getInstance() {
		if (m_Binder == null) {
			m_Binder = new IntBuilder();
		}
		return m_Binder;
	}

	public Integer getValue(Object objCurrent, String strJavaField) {
		try {
			Method mt = objCurrent.getClass().getMethod("get" + strJavaField);
			return (Integer) mt.invoke(objCurrent);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		return null;
