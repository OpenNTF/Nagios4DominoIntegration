...
import biz.webgate.xpages.dss.binding.Definition;
...
public class DominoStorageService {
	...
	private Object getObjectID(DominoStore ds, Object obj) {
		try {
			if (ds.PrimaryFieldClass().equals(String.class)) {
				return Definition.STRING.getValue(obj, ds.PrimaryKeyField());
			}
			if (ds.PrimaryFieldClass().equals(Integer.class)) {
				return Definition.INTEGER.getValue(obj, ds.PrimaryKeyField());
			}
			if (ds.PrimaryFieldClass().equals(Double.class)) {
				return Definition.DOUBLE.getValue(obj, ds.PrimaryKeyField());
			}
			if (ds.PrimaryFieldClass().equals(Double[].class)) {
				return Definition.DOUBLE_ARR
						.getValue(obj, ds.PrimaryKeyField());
			}
			if (ds.PrimaryFieldClass().equals(Double.class)) {
				return Definition.LONG.getValue(obj, ds.PrimaryKeyField());
			}
			if (ds.PrimaryFieldClass().equals(Date.class)) {
				return Definition.DATE.getValue(obj, ds.PrimaryKeyField());
			}
			if (ds.PrimaryFieldClass().equals(String[].class)) {
				return Definition.STRING_ARR.getValue(obj, ds.PrimaryKeyField());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private Java2DominoBinder buildSaveBinder(DominoStore dsStore, Class<?> currentClass) {
		...
		for (Field fldCurrent : currentClass.getDeclaredFields()) {
			if (fldCurrent.isAnnotationPresent(DominoEntity.class)) {
				...
				if (!de.readOnly()) {
					IBinder<?> binder = Definition.getBinder(fldCurrent.getType());
					if (binder != null) {						
						...
						j2dRC.addDefinition(de.FieldName(), buildCleanFieldName(dsStore, fldCurrent.getName()), binder, addValues);
					}
				}
			}
		}
	...
	private Domino2JavaBinder buildLoadBinder(DominoStore dsStore, Class<?> currentClass) {
		...
		for (Field fldCurrent : currentClass.getDeclaredFields()) {
			if (fldCurrent.isAnnotationPresent(DominoEntity.class)) {
				...
				if (!de.writeOnly()) {
					if (de.isFormula()) {
						IBinder<?> binder = Definition.getFormulaBinder(fldCurrent.getType());
						if (binder != null) {
							djdRC.addDefinition(de.FieldName(), buildCleanFieldName(dsStore, fldCurrent.getName()), binder, null);
						}
					} else {
						IBinder<?> binder = Definition.getBinder(fldCurrent.getType());
						HashMap<String, Object> addValues = new HashMap<String, Object>();
						...
						if (binder != null) {
							djdRC.addDefinition(de.FieldName(), buildCleanFieldName(dsStore, fldCurrent.getName()), binder, addValues);
						}
					}
				}
			}
	...
	private String buildCleanFieldName(DominoStore dsCurrent, String fldName) {
		String fldNameClean = fldName;
		if (dsCurrent.JavaFieldPrefix().length() > 0) {
			fldNameClean = fldNameClean.substring(dsCurrent.JavaFieldPrefix().length());
		}
		if (dsCurrent.JavaFieldPostFix().length() > 0) {
			fldNameClean = fldNameClean.substring(0, (fldNameClean.length() - dsCurrent.JavaFieldPostFix().length()));
		}
		return fldNameClean;
	}
}
